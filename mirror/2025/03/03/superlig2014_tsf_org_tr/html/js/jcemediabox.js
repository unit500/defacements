...
return;this.domLoaded=true;var t=this,na=navigator,ua=na.userAgent;t.isOpera=window.opera&&opera.buildNumber;t.isWebKit=/WebKit/.test(ua);t.isChrome=/Chrome\//.test(ua);t.isSafari=/Safari\//.test(ua);t.isIE=!t.isWebKit&&!t.isOpera&&(/MSIE/gi).test(ua)&&(/Explorer/gi).test(na.appName)&&!!window.ActiveXObject;t.isIE6=t.isIE&&/MSIE [56]/.test(ua)&&!window.XMLHttpRequest;t.isIE7=t.isIE&&/MSIE [7]/.test(ua)&&!!window.XMLHttpRequest&&!document.querySelector;t.isIDevice=/(iPad|iPhone)/.test(ua);t.isAndroid=/Android/.test(ua);this.site=this.getSite();if(!this.site)
...
return zoom;},auto:function(){var t=this,expires=JCEMediaBox.options.popup.cookie_expiry,dts,key;function makeID(){var url=document.location.href;var key=window.btoa(url);key=key.replace(/[^\w]/g,'');key=key.substr(0,24);return key;}
JCEMediaBox.each(this.popups,function(el,i){if(el.auto){if(el.auto=='single'){key=el.id||makeID();var cookie=t.getCookie('jcemediabox_'+key+'_'+i);if(!cookie){if(expires){dts=new Date();dts.setHours(expires*24);}
...
extend(this.active,{'src':p.src||o.src,'title':o.title||p.title||'','caption':p.caption||'','type':p.type||this.getType(o),'params':p||{},'width':width,'height':height});this.info();switch(this.active.type){case'image':case'image/jpeg':case'image/png':case'image/gif':case'image/bmp':if(this.print&&this.options.print){this.print.style.visibility='visible';}
...
if(t.active.type=='image'&&!JCEMediaBox.isIE6){FX.animate(t.content,{'opacity':1},fs,function(){showInfo();});}else{DOM.style(t.content,'opacity',1);showInfo();}});},close:function(keepopen){var t=this,each=JCEMediaBox.each,DOM=JCEMediaBox.DOM;if(this.iframe){DOM.attribute(this.iframe,'src','');}
